...
function get_aff()
    da = S3Dict("s3://neuroglancer/pinky40_v11/semanticmap-5/4_4_40/")
    aff = BigArray(da)
    return aff
end

function get_seg()
    ds = GSDict("gs://neuroglancer/pinky40_v11/watershed_mst_trimmed_sem_remap/4_4_40/")
    seg = BigArray(ds)
    return seg
end
...
if ARGS[1] == "aff"
    aff = get_aff()[aff_start[1]:aff_end[1], aff_start[2]:aff_end[2], aff_start[3]:aff_end[3], 1:4]
...
if ARGS[1] == "seg"
    ...
    f["main"] = new_seg[1+cut_before[1]:end-cut_after[1], 1+cut_before[2]:end-cut_after[2],1+cut_before[3]:end-cut_after[3]]
    close(f)
end
