            ...
            open("$(ARGS[2])/$(p[1])_$(p[2])_$(index[1])_$(index[2])_$(index[3]).txt", "w") do f
                for i in 1:3
                    for k in keys(edges[p].boundaries[i])
                        write(f, "$i $(k[1]+offset[1]-one(Int32)) $(k[2]+offset[2]-one(Int32)) $(k[3]+offset[3]-one(Int32)) $(Float64(edges[p].boundaries[i][k][1][1])) $(Float64(edges[p].boundaries[i][k][1][2])) $(Float64(edges[p].boundaries[i][k][1][3])) $(Float64(edges[p].boundaries[i][k][1][4])) $(Float64(edges[p].boundaries[i][k][2][1])) $(Float64(edges[p].boundaries[i][k][2][2])) $(Float64(edges[p].boundaries[i][k][2][3])) $(Float64(edges[p].boundaries[i][k][2][4]))\n")
                    end
                end
            end
