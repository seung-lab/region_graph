...
module SQLEdges
    ...
    function get_incomplete_edges{Ta, Ts}(segment_pairs::Array{ Tuple{Ts, Ts}, 1 }, aff_threshold::Ta)
        ...
        for segment_pairs_offset in 1:5000:length(segment_pairs)
            ...
            const command = string("SELECT seg_id_1, seg_id_2, voxels FROM edge_voxel_list WHERE seg_id_1 = ? AND seg_id_2 = ?",
                                ...
                                ";")
            const querytypes = fill(MYSQL_TYPE_LONGLONG, 2 * length(segment_pairs_chunk))
            const queryvalues = reinterpret(Ts, segment_pairs_chunk)
    ...
    function insert_incomplete_edges{Ta, Ts}(edges::Array{MeanEdge{Ts, Ta}, 1}, startIndex::Array{Int32, 1})
        ...
        for edges_offset in 1:5000:length(edges)
            ...
            queryvalues = Array{Any, 1}(length(edges_chunk))
            ...
            for (idx, edge) in enumerate(edges_chunk)
                ...
                queryvalues[(4*idx - 0)] = string("0x", map(char->hex(char, 2), read(hexBuffer)))
            end
